...
int main(int argc, char* argv[]) {
    ...
    if (sem_init(mutex, 0, 1) != 0) { // Creating mutex for delimitation access to critical expansion
        fprintf(stderr, "Error by creating semaphore... Try 'make phil2'\n");
        return 3;
    }    
    ...
    int i;
    for (i = 0; i < N; i++) {
        if (sem_init(&eaters[i], 0, 0) != 0) {  // Creating N semaphores
            fprintf(stderr, "Error by creating semaphore... Try 'make phil2'\n");
            return 3;
        }
    }
